...
public class JsonStateWriter implements GameStateWriter {
    ...
    public void write(Board board, List<Player> players, List<RoadPlacement> roadOrder) {
        ...
        for (RoadPlacement placement : roadOrder) {
            Edge edge = board.getEdge(placement.getEdgeId());
            if (edge == null || edge.getRoadOwnerId() == Edge.UNOWNED) {
                continue;
            }
            Player owner = playersById.get(placement.getOwnerPlayerId());
            if (owner == null) {
                continue;
            }
            if (!firstRoad) {
